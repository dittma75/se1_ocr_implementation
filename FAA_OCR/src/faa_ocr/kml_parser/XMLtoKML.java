...
{
    //Main String to be written to the kml file
	private String kml_string = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>";
	//String path to be defined and returned
		...
		for (int coord = 0; coord < coord_list.getLength(); coord++) {
			...
			Node coord_node = coord_list.item(0);
				...
				if (coord_node.getNodeType() == Node.ELEMENT_NODE) {
					...
					//Now, for all three types of path nodes, loop through and add every node's
					// coordinates to the local coord_string
					for(int node_index = 0; node_index < node_list.getLength(); node_index++){
						...
						if (node.getNodeType() == Node.ELEMENT_NODE) {
							...
							coord_string += "\n\t\t "
									+ node_element.getElementsByTagName("longitude").item(0)
											.getTextContent()
									+ ","
									+ node_element.getElementsByTagName("latitude").item(0)
